...
export default function TikTokShopConnectButton({
  onConnectionSuccess,
  onConnectionError,
  className = '',
  size = 'md'
}: TikTokShopConnectButtonProps) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/tiktokshop/auth?wallet=${encodeURIComponent(address)}`
      );
      const data = await response.json();

      if (!response.ok) {
        throw new Error(data.error || 'Failed to initiate TikTok Shop connection');
      }

      // Redirect to TikTok Shop OAuth
      window.location.href = data.authUrl;
    } catch (error) {
      console.error('TikTok Shop connection error:', error);

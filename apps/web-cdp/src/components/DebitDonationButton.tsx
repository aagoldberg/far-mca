...
export const DebitDonationButton: React.FC<DebitButtonProps> = ({ fiatAmount, campaignNumericId, onSuccess, onError }) => {
    ...
    if (error || !COINBASE_APP_ID) {
        return (
            <button disabled className="w-full mt-2 bg-gray-300 text-white font-bold py-3 px-4 rounded-lg cursor-not-allowed">
                {error ? "Error loading campaign" : "Fiat Donations Unavailable"}
            </button>
        );
    }
